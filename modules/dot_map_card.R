#' 
#' Dot Distribution Map Card Module
#'
#' This script contains two functions:
#' \code{dot_map_ui} generates the user interface for each card
#' \code{card_dot_map} generates the plot shown in a card
#' 
...
#' 
#' @details
#' \code{dot_map_ui} needs the text that will surround the map on 
...
#' 
#' @details
#' \code{card_dot_map} needs the data, fields, colors, and labels for the map
#' 
...
card_dot_map <- function(input,
  ...
  output$plot <- renderLeaflet({
    ...
    popup_text <- paste0(selected_data()[[popup_label]], 
                        ": ",
                        selected_data()[[popup_value]],
                        " ",
                        popup_units)
    ...
    leaflet(selected_data(),
      ...
      addCircleMarkers(lng =  selected_data()[[lon_field]], 
                       lat = selected_data()[[lat_field]],
                       radius = ~35*sqrt(selected_data()[[chart_width]]) / sqrt(max(selected_data()[[chart_width]])),
                       stroke = FALSE, 
                       color = col,
                       fillOpacity = 0.5,
                       label = popup_text,
                       labelOptions = 
                         labelOptions( 
                           style=list('font-size' = label_font_size)
                           )) %>% 
